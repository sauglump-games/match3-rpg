...
const ALL_CLASSES = ['warrior', 'cleric', 'mage', 'rogue', 'druid', 'paladin', 'ranger'];
const MARTIAL = ['warrior', 'paladin', 'rogue', 'ranger'];
...
export const ITEMS = {
    ...
    'dagger': {
        ...
        damageDie: { n: 1, sides: 4 },
        description: '1d4 + 1. Anyone may wield it.',
    },
    'longsword': {
        id: 'longsword', name: 'Longsword', slot: 'weapon', rarity: 'common',
        classes: [...MARTIAL],
        bonuses: { damageBonus: 2 },
        damageDie: { n: 1, sides: 8 },
        description: '1d8 + 2. For warriors, paladins, rogues, rangers.',
    },
    'longsword-plus-1': {
        ...
        damageDie: { n: 1, sides: 8 },
        description: '1d8 + 3. Magical weapon.',
    },
    'mace': {
        id: 'mace', name: 'Mace', slot: 'weapon', rarity: 'common',
        classes: [...new Set([...MARTIAL, ...FAITHFUL])],
        bonuses: { damageBonus: 2 },
        damageDie: { n: 1, sides: 6 },
        description: '1d6 + 2. Blunt — clerics may wield it.',
    },
    'quarterstaff': {
        ...
        bonuses: { damageBonus: 1 },
        damageDie: { n: 1, sides: 6 },
        description: '1d6 + 1. For mages, clerics, druids.',
    },
    // ----- armor -----
    'leather-armor': {
    ...
    'shortbow-plus-1': {
        ...
        bonuses: { damageBonus: 3 },
        damageDie: { n: 1, sides: 6 },
        description: '1d6 + 3. Quick and accurate.',
    },
    'warhammer': {
    ...
    'sword-of-vengeance': {
        ...
        damageDie: { n: 1, sides: 8 },
        description: '1d8 + 4. Demands a price the wielder rarely sees coming.',
    },
    'wand-of-magic-missiles': {
        ...
        damageDie: { n: 1, sides: 4 },
        description: '1d4 + 3, +1 L1 spell slot. Mage focus.',
    },
    'sun-blade': {
        ...
        damageDie: { n: 1, sides: 8 },
        description: '1d8 + 5. A radiant blade that scorches the dark.',
    },
...
/**
 * AD&D 2e encumbrance (spec §1.1 weight / §6 P9). Items carry a `weight`
 * field (coins). The party hero is "encumbered" when total weight exceeds
 * `strWeightLimit(STR)`. We skip the item if weight is missing (legacy /
 * trivial items), and gear without a weight property contributes 0.
 *
 * Default fall-back weights by slot, used when an item omits `weight`:
 *   weapon 20, armor 250 (plate) / 40 (leather) — we key off armorBonus
 *   to pick a sensible proxy. Shield 30. Accessory 5.
 */
function itemWeight(item) {
    if (typeof item.weight === 'number') return item.weight;
    if (item.slot === 'armor') {
        const ab = item.bonuses?.armorBonus ?? 0;
        if (ab >= 5) return 250;   // plate / full plate
        if (ab >= 3) return 120;   // half plate / chain
        return 40;                 // leather / studded
    }
    if (item.slot === 'weapon') return 20;
    if (item.slot === 'shield') return 30;
    return 5;
}

export function totalEquipmentWeight(hero) {
    let total = 0;
    ...
    // Carried-but-not-equipped gear still counts.
    for (const item of hero?.inventory ?? []) total += itemWeight(item);
    return total;
}

/**
 * Damage-die descriptor for the hero's equipped weapon per spec §3. Returns
 * `null` when unarmed — callers use that to skip the die roll so unarmed
 * heroes keep the score-only damage profile (the codebase started without
 * weapon dice, and a silent 1d2 would break every fixed-damage test).
 */
export function weaponDamageDie(hero) {
    const weapon = hero?.equipment?.weapon;
    return weapon?.damageDie ?? null;
}
