...
import { normalizeDailyUses } from './class-features.js';
import { defaultMageKnownSpells } from './spells.js';
...
export function serializeGame(game) {
    ...
    // Mage spellbooks are Sets — downgrade to sorted arrays in the save. The
    // hydrate pass below re-upgrades them. Applied after the clone so the
    // live Game instance keeps its Sets.
    if (Array.isArray(out.party)) {
        for (const hero of out.party) {
            if (hero?.role === 'mage' && game.party) {
                const live = game.party.find(p => p.id === hero.id);
                if (live?.knownSpells instanceof Set) {
                    hero.knownSpells = [...live.knownSpells].sort();
                } else delete hero.knownSpells;
            }
        }
    }
    ...
    out.encounter = game.encounter ? {
        ...
        turnUndeadUsed: [...(game.encounter._turnUndeadUsed ?? [])],
    } : null;
...
export function hydrateGame(game, data) {
    ...
    for (const hero of game.party ?? []) {
        ...
        // Phase 5 migration: per-day class-feature usage. Fresh roster =
        // full charges; extant saves preserve what was already spent.
        hero.dailyUses = normalizeDailyUses(hero.dailyUses, hero.role, hero.level ?? 1);
        // P5: mage spellbooks (Set<spellId>). Legacy saves skip the field and
        // hydrate to the default known-set at the hero's current level.
        if (hero.role === 'mage') {
            hero.knownSpells = Array.isArray(hero.knownSpells)
                ? new Set(hero.knownSpells)
                : defaultMageKnownSpells(hero.level ?? 1);
        }
    }
    ...
    if (data.encounter) {
        ...
        game.encounter._turnUndeadUsed = new Set(data.encounter.turnUndeadUsed ?? []);
    } else {
        game.encounter = null;
    }
