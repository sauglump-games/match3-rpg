...
import { projectedAction } from './combat.js';
import {
...
export function renderParty(partyEl, party, { onHeroClick, scoreByColor = {} } = {}) {
    partyEl.innerHTML = '';
...
function renderItemBonuses(item) {
    ...
    if (b.maxManaBonus)parts.push(`+${b.maxManaBonus} MP`);
    return parts.join(', ');
}
...
export function renderQuestLog(overlayEl, quests, { onClose }) {
    overlayEl.innerHTML = '';
    overlayEl.classList.add('visible');
    const panel = document.createElement('div');
...
export function renderRecruitOverlay(overlayEl, recruit, party, maxPartySize, { onAccept, onDecline, onReplace }) {
    ...
    const stats = `
        <div class="recruit-stats">
            ...
            ${recruit.maxMana > 0 ? `<div>Mana: <strong>${recruit.mana} / ${recruit.maxMana}</strong></div>` : ''}
