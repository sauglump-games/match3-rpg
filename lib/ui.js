...
import { dailyUsesRemaining, effectiveTurnLevel } from './class-features.js';
import {
...
function popEventFloat(ev) {
    switch (ev.type) {
        ...
        case 'turn-undead-start': {
            const anchor = actorAnchor(ev.sourceId);
            if (!anchor) return;
            popFloat({ x: anchor.x, y: anchor.y, text: '✨ TURN UNDEAD', classes: 'float-spell' });
            return;
        }
        case 'turn-undead-result': {
            const anchor = actorAnchor(ev.targetId);
            if (!anchor) return;
            const text = ev.outcome === 'destroy' ? 'DESTROYED' : ev.outcome === 'turn' ? 'TURNED' : 'RESIST';
            const cls = ev.outcome === 'fail' ? 'float-miss' : 'float-cleanse';
            popFloat({ x: anchor.x, y: anchor.y, text, classes: cls });
            return;
        }
        case 'lay-on-hands': {
            const anchor = actorAnchor(ev.sourceId);
            if (!anchor) return;
            popFloat({ x: anchor.x, y: anchor.y, text: '✋ LAY ON HANDS', classes: 'float-spell' });
            return;
        }
        case 'shapechange': {
            const anchor = actorAnchor(ev.sourceId);
            if (!anchor) return;
            popFloat({ x: anchor.x, y: anchor.y, text: '🐺 SHAPECHANGE', classes: 'float-spell' });
            return;
        }
        case 'raise-dead-attempt': {
            const anchor = actorAnchor(ev.targetId);
            if (!anchor) return;
            const text = ev.survived ? `RAISE: ${ev.roll}/${ev.target}` : `FAIL: ${ev.roll}/${ev.target}`;
            const cls = ev.survived ? 'float-heal' : 'float-save-fail';
            popFloat({ x: anchor.x, y: anchor.y, text, classes: cls });
            return;
        }
        case 'raise-dead-revived': {
            const anchor = actorAnchor(ev.targetId);
            if (!anchor) return;
            popFloat({ x: anchor.x, y: anchor.y, text: '✨ REVIVED', classes: 'float-heal' });
            return;
        }
        case 'raise-dead-refused': {
            const anchor = actorAnchor(ev.targetId);
            if (!anchor) return;
            popFloat({ x: anchor.x, y: anchor.y, text: 'NO RAISE (elf)', classes: 'float-miss' });
            return;
        }
        default:
            return;
...
// Each combat event's impact on an actor's displayed (hp, shield, alive).
// `null` means the event is purely informational (miss / save / spell-cast /
// level-up / status-applied with no hp change / etc.) and the animator skips
// it.  hpDelta is a SIGNED change against pre-event state; shieldDelta likewise.
// When multiple actors change on one event (e.g., emerald heals the whole
// party? — actually each heal fires its own event — so single-actor changes
// suffice here), we return a single { id, hpDelta, shieldDelta, killOnZero }.
function eventActorDelta(ev) {
    switch (ev.type) {
        case 'damage':
            // `amount` is post-absorb HP damage; `absorbed` is shield loss.
            return { id: ev.targetId, hpDelta: -(ev.amount ?? 0), shieldDelta: -(ev.absorbed ?? 0), killOnZero: true };
        case 'enemy-attack':
            return { id: ev.targetId, hpDelta: -(ev.amount ?? 0), shieldDelta: -(ev.absorbed ?? 0), killOnZero: true };
        case 'heal':
            return { id: ev.targetId, hpDelta: +(ev.amount ?? 0), shieldDelta: 0, killOnZero: false };
        case 'shield':
            return { id: ev.targetId, hpDelta: 0, shieldDelta: +(ev.amount ?? 0), killOnZero: false };
        case 'enemy-defend':
            return { id: ev.sourceId, hpDelta: 0, shieldDelta: +(ev.amount ?? 0), killOnZero: false };
        case 'status-tick':
            return ev.kind === 'poison'
                ? { id: ev.targetId, hpDelta: -(ev.amount ?? 0), shieldDelta: 0, killOnZero: true }
                : null;
        default:
            return null;
    }
...
// Patch one actor card's HP bar + shield chip from a running state
// `{ hp, maxHp, shield, alive }`. Non-fatal if the card isn't in DOM.
function patchActorCard(id, state) {
    const card = document.querySelector(`[data-id="${id}"]`);
    if (!card) return;
    const bar = card.querySelector('.hp-bar-fill');
    const text = card.querySelector('.hp-bar-text');
    if (bar) {
        const pct = Math.max(0, Math.min(100, (state.hp / state.maxHp) * 100));
        bar.style.width = `${pct}%`;
    }
    if (text) text.textContent = `${state.hp}/${state.maxHp}`;
    // Shield chip is an optional child; rebuild it inline to match current value.
    const existingShield = card.querySelector('.shield');
    if (state.shield > 0) {
        if (existingShield) existingShield.textContent = `🛡 ${state.shield}`;
    } else if (existingShield) {
        existingShield.remove();
    }
    if (!state.alive) card.classList.add('dead');
}

/**
 * Snapshot displayed combat state (hp, maxHp, shield, alive) for every
 * party member and enemy. The caller captures this BEFORE mutating the
 * game and passes it to `animateCombatEvents` so damage can be replayed
 * one actor at a time.
 */
export function snapshotActorState(party, enemies) {
    const out = new Map();
    for (const h of party ?? []) out.set(h.id, { hp: h.hp, maxHp: h.maxHp, shield: h.shield, alive: h.alive });
    for (const e of enemies ?? []) out.set(e.id, { hp: e.hp, maxHp: e.maxHp, shield: e.shield, alive: e.alive });
    return out;
}

/**
 * Replay combat events through the DOM, patching hero / enemy cards one
 * event at a time with `stagger` ms between impactful events. The game's
 * state is already final — this animation gives the player a readable
 * "one after another" impact sequence. Call `onDone` after the last
 * event so the caller can trigger a final full render that resyncs any
 * non-event state (shield decay, status icons, level-ups).
 */
export function animateCombatEvents(events, preState, { stagger = FLOAT_STAGGER_MS, onDone } = {}) {
    const running = new Map();
    for (const [id, s] of preState) running.set(id, { ...s });
    let delay = 0;
    for (const ev of events) {
        const delta = eventActorDelta(ev);
        if (!delta) continue;
        const evDelay = delay;
        setTimeout(() => {
            const s = running.get(delta.id);
            if (!s) return;
            s.shield = Math.max(0, s.shield + delta.shieldDelta);
            s.hp = Math.max(0, Math.min(s.maxHp, s.hp + delta.hpDelta));
            if (delta.killOnZero && s.hp === 0) s.alive = false;
            patchActorCard(delta.id, s);
        }, evDelay);
        delay += stagger;
    }
...
/**
 * Class-feature offers for a hero — populated at render time from
 * role + level + remaining per-day uses. Paladin shows up here even though
 * they don't cast spells until L9, so they can trigger Lay on Hands.
 */
function featureOffersFor(hero) {
    const offers = [];
    if (hero.role === 'cleric' || (hero.role === 'paladin' && (hero.level | 0) >= 3)) {
        const lvl = effectiveTurnLevel(hero);
        offers.push({
            id: 'turn-undead',
            name: 'Turn Undead',
            costLabel: '1× combat',
            description: `Channel holy power to rout undead (cleric-level ${lvl}).`,
        });
    }
    if (hero.role === 'paladin') {
        const max = hero.dailyUses?.max?.layOnHands ?? 0;
        const remaining = dailyUsesRemaining(hero, 'layOnHands');
        offers.push({
            id: 'lay-on-hands',
            name: 'Lay on Hands',
            costLabel: `${remaining}/${max}·day`,
            description: `Heal the most-wounded ally for ${2 * (hero.level | 0)} HP.`,
        });
    }
    if (hero.role === 'druid' && (hero.level | 0) >= 7) {
        const max = hero.dailyUses?.max?.shapechange ?? 0;
        const remaining = dailyUsesRemaining(hero, 'shapechange');
        offers.push({
            id: 'shapechange',
            name: 'Shapechange',
            costLabel: `${remaining}/${max}·day`,
            description: 'Shift into a beast form — heal 1d6 × 10% max HP.',
        });
    }
    return offers;
}

// WoW-style action-bar tile glyphs. We pick a compact pictogram per spell
// so the bottom bar reads as icons at a glance. Fallback = class glyph.
const SPELL_GLYPH = {
    'magic-missile':        '✦',
    'sleep':                '💤',
    'shocking-grasp':       '⚡',
    ...
    'paladin-bless':        '✝️',
    'paladin-protection-from-evil': '🛡️',
    'paladin-aid':          '💛',
    'paladin-slow-poison':  '🫗',
    'ranger-entangle':      '🌿',
...
function spellGlyph(spellId, fallback = '🪄') {
    return SPELL_GLYPH[spellId] ?? fallback;
}

/**
 * Render the bottom action bar — a horizontal, WoW-style row of square
 * icon tiles grouped per caster. Each tile is a clickable spell or
 * class-feature slot. The `canCast` / `canUseFeature` callbacks drive the
 * disabled state so tiles dim when the score / slot / daily-use gates
 * aren't met. Non-casters without features render nothing, and the whole
 * bar hides on non-combat phases via the caller's `.hidden` toggle.
 */
export function renderSpellTray(trayEl, party, { canCast, onCast, canUseFeature, onUseFeature } = {}) {
    trayEl.innerHTML = '';
    const entries = party.filter(p => p.alive && (isCasterRole(p.role) || featureOffersFor(p).length > 0));
    if (entries.length === 0) {
    ...
    for (const hero of entries) {
        ...
        header.textContent = `${hero.name} L${hero.level}${slotSummary ? ' · ' + slotSummary : ''}`;
        ...
        for (const spell of hero._spells ?? []) {
            ...
            btn.title = `${spell.name} — ${spell.description}\n(1× L${spell.spellLevel} slot · ${spell.scoreCost}${colorGlyph})`;
            btn.dataset.spellId = spell.id;
            btn.dataset.casterId = hero.id;
            btn.innerHTML = `
                <span class="spell-name">${spellGlyph(spell.id)}</span>
                <span class="spell-label">${spell.name}</span>
                <span class="spell-cost">L${spell.spellLevel}·${spell.scoreCost}${colorGlyph}</span>
            `;
            btn.disabled = !canCast(spell.id, hero.id);
            btn.addEventListener('click', () => onCast(spell.id, hero.id));
            list.appendChild(btn);
        }
        for (const feature of featureOffersFor(hero)) {
            const btn = document.createElement('button');
            btn.className = 'spell-btn feature-btn';
            btn.title = `${feature.name} — ${feature.description}\n(${feature.costLabel})`;
            btn.dataset.featureId = feature.id;
            btn.dataset.casterId = hero.id;
            btn.innerHTML = `
                <span class="spell-name">${FEATURE_GLYPH[feature.id] ?? '✨'}</span>
                <span class="spell-label">${feature.name}</span>
                <span class="spell-cost">${feature.costLabel}</span>
            `;
            btn.disabled = !canUseFeature?.(feature.id, hero.id);
            btn.addEventListener('click', () => onUseFeature?.(feature.id, hero.id));
            list.appendChild(btn);
        }
 ...
 */
/**
 * Render a compact list of class features this hero has access to, with
 * their current daily / per-combat budget. Returns '' when the hero has
 * no class features (most heroes — warriors, mages, rogues).
 */
function renderFeatureBlock(hero) {
    const lines = [];
    if (hero.role === 'cleric' || (hero.role === 'paladin' && (hero.level | 0) >= 3)) {
        const lvl = effectiveTurnLevel(hero);
        lines.push(`<li><strong>Turn Undead</strong> <span class="dim">(1×/combat, as cleric L${lvl})</span></li>`);
    }
    if (hero.role === 'paladin') {
        const max = hero.dailyUses?.max?.layOnHands ?? 1;
        const remaining = dailyUsesRemaining(hero, 'layOnHands');
        lines.push(`<li><strong>Lay on Hands</strong> <span class="dim">(${remaining}/${max} per day · ${2 * (hero.level | 0)} HP)</span></li>`);
    }
    if (hero.role === 'druid' && (hero.level | 0) >= 7) {
        const max = hero.dailyUses?.max?.shapechange ?? 3;
        const remaining = dailyUsesRemaining(hero, 'shapechange');
        lines.push(`<li><strong>Shapechange</strong> <span class="dim">(${remaining}/${max} per day · heal 1d6 × 10% max HP)</span></li>`);
    }
    if (hero.role === 'rogue') {
        const mult = (hero.level | 0) >= 13 ? 5 : (hero.level | 0) >= 9 ? 4 : (hero.level | 0) >= 5 ? 3 : 2;
        lines.push(`<li><strong>Backstab</strong> <span class="dim">(passive · ×${mult} damage on successful positioning)</span></li>`);
    }
    if (lines.length === 0) return '';
    return `<ul class="char-sheet-features">${lines.join('')}</ul>`;
}
...
export function renderCharacterSheet(overlayEl, hero, { onClose, onEquip, onUnequip, canEquipItem }) {
    ...
    const featuresHtml = renderFeatureBlock(hero);
    const statsTab = `
        <div class="char-sheet-tab active" data-tab="stats">
            ...
            ${featuresHtml ? `<h3 class="char-sheet-section">Class Features</h3>${featuresHtml}` : ''}
        </div>
    `;
...
/**
 * Surface the persistent combat log as a modal overlay. The live
 * `<ul id="log">` accumulates entries via `renderLog`; this call clones
 * it into the overlay so the modal always reflects the latest state
 * without duplicating log state.
 */
export function renderLogOverlay(overlayEl, logEl, { onClose } = {}) {
    overlayEl.innerHTML = '';
    overlayEl.classList.add('visible');
    const panel = document.createElement('div');
    panel.className = 'rest-panel log-modal';
    const header = document.createElement('h2');
    header.textContent = 'Combat Log';
    panel.appendChild(header);
    // Clone the live log into the panel so the overlay view matches
    // exactly what renderLog has been appending.
    const cloned = logEl.cloneNode(true);
    cloned.removeAttribute('id');
    cloned.classList.remove('log-hidden');
    if (cloned.children.length === 0) {
        const empty = document.createElement('div');
        empty.className = 'log-empty';
        empty.textContent = 'No combat actions yet.';
        panel.appendChild(empty);
    } else {
        panel.appendChild(cloned);
    }
    const footer = document.createElement('div');
    footer.className = 'rest-actions';
    const closeBtn = document.createElement('button');
    closeBtn.className = 'primary log-close';
